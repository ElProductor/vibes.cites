...
export class VibeQuantumAI {
     ...
     */
    private calculateAuraSynergy(colorA: string, colorB: string): number {
        if (!colorA || !colorB) return 50; // Neutral si faltan datos
        ...
        if (diff <= 30) return 85 + (30 - diff) / 2; 
        if (diff >= 150) return 90 + (diff - 150) / 3; 
        if (diff >= 75 && diff <= 105) return 40; // Choque
     ...
     */
    private calculateAstroSynergy(signA: string, signB: string): number {
        const elements: any = {
        ...
        if (!elA || !elB) return 60;
        if (elA === elB) return 95; // Mismo elemento (Profunda conexión)
        
        // Complementarios cósmicos
        if ((elA === 'Fire' && elB === 'Air') || (elA === 'Air' && elB === 'Fire')) return 90;
        if ((elA === 'Earth' && elB === 'Water') || (elA === 'Water' && elB === 'Earth')) return 90;
        
        // Elementos en tensión
        if ((elA === 'Fire' && elB === 'Water') || (elA === 'Water' && elB === 'Fire')) return 45;
        if ((elA === 'Earth' && elB === 'Air') || (elA === 'Air' && elB === 'Earth')) return 50;
     ...
     */
    private analyzeBioLinguistics(bioA: string, bioB: string): number {
        if (!bioA || !bioB) return 50;

        const clean = (text: string) => text.toLowerCase().replace(/[^\w\sáéíóú]/g, '').split(' ').filter(w => w.length > 3);
        const wordsA = new Set(clean(bioA));
        const wordsB = new Set(clean(bioB));
        
        if (wordsA.size === 0 || wordsB.size === 0) return 50;
     ...
     */
    public enhanceBio(currentBio: string, zodiac: string): string {
     ...
     */
    public evaluateEventSuitability(userZodiac: string, eventVibe: string): string {
     ...
     */
    public generateIcebreaker(synergy: number, insight: string): string {
        if (synergy > 85) return `¡Nuestras auras están en ${synergy}%! ${insight} ¿Qué te apasiona últimamente?`;
        if (synergy > 70) return `Tenemos buena resonancia. ${insight} ¿Cuál es tu plan para el finde?`;
     ...
     */
    public generateDailyForecast(zodiac: string): string {
        const forecasts = [
...
export const vibeAI = new VibeQuantumAI();