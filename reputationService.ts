...
export class ReputationService {
   ...
   */
  async processBehavior(userId: string, action: 'ATTENDED' | 'NO_SHOW' | 'LATE' | 'REPORTED' | 'GOOD_VIBE_BADGE') {
    let scoreChange = 0;
    ...
    try {
      ...
      // 1. Registrar en Log
      await db.query(
        `INSERT INTO vibe_score_log (user_id, change_amount, reason) VALUES ($1, $2, $3)`,
        [userId, scoreChange, action]
      );

      // 2. Actualizar Usuario (con límite mínimo de 0 y máximo de 1000)
      await db.query(
        `UPDATE users SET vibe_score = MAX(0, MIN(1000, vibe_score + $1)) WHERE id = $2`,
        [scoreChange, userId]
      );

      // 3. Verificar y Desbloquear Insignias (Badges) automáticamente
      await this.checkAndUnlockBadges(db, userId);
  ...
  private async checkAndUnlockBadges(client: any, userId: string) {
    // Obtener datos actuales del usuario (Score y Eventos asistidos)
    const userRes = await client.query(
      `SELECT u.vibe_score, COUNT(ea.event_id) as event_count 
       FROM users u 
    ...
    const newBadges = await client.query(
      `SELECT id, name, icon_url FROM badges WHERE id IN (
    ...
    for (const badge of newBadges.rows) {
      ...
      await this.notificationService.notifyUser(
        ...
        `Has ganado la insignia: ${badge.name}`,
        { badgeId: badge.id, icon: badge.icon_url }
      );
    }
  }
  ...
  async getLeaderboard(limit: number = 10) {
    const res = await db.query(
      `SELECT id, username, vibe_score FROM users 
       ORDER BY vibe_score DESC LIMIT $1`,
      [limit]
    );
    return res.rows;
  }
  ...
  async getUserRank(userId: string) {
    ...
    const res = await db.query(
      `SELECT COUNT(*) + 1 as rank FROM users WHERE vibe_score > (SELECT vibe_score FROM users WHERE id = $1)`,
      [userId]
    );
